...
public class MemberAWT extends MFrame implements ActionListener {
	...
	public void insertForm() {
		...
		Vector<String> vlist = mgr.listTeam();
		ch.add("팀을 선택하세요.");
		for (int i = 0; i < vlist.size(); i++) {
			ch.add(vlist.get(i));
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == b1 /* 수정버튼 */ ) {
			...
			int i = list.getSelectedIndex(); // 선택된 item 번호
			if (i == -1) return; // 메소드에서 중간에 return은 반복문의 break 역할
			MemberBean bean = vlist.get(i);
